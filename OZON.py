...
def add_tobaza():
    fileName = filedialog.askopenfilename()
    df_to_append = pd.read_excel(f'{fileName}-На перевод.xlsx')
...
def LD_work():
    df_baza_LD = pd.read_excel('C:/Users/User/Desktop/РЕЕСТРЫ/БАЗА ЛД.xlsx', sheet_name=0, engine='openpyxl',
                               header=None, usecols='A,C', skiprows=1)
    df_baza_LD = df_baza_LD.rename(columns={0: 'SKU', 2: 'good_description'})
    fileName = filedialog.askopenfilename()
    wb = openpyxl.load_workbook(fileName)
    ws = wb.active
    wb2 = openpyxl.load_workbook(f'{fileName}-На перевод.xlsx')
    ws2 = wb2.active
    df_translate_table = pd.read_excel(f'{fileName}-На перевод.xlsx-translate_table.xlsx', sheet_name=0, engine='openpyxl', header=None, usecols='B')
    ...
    df_LD_registry = pd.read_excel(fileName, sheet_name=0, engine='openpyxl', header=None, usecols='Y', skiprows=1)
...
button5 = tk.Button(text="Проверка паспорт + запрещенка", width=35, height=2, bg="lightgrey", fg="black", command=passport_check)
